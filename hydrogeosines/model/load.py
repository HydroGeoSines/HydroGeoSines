...
class Load(Tools):
    ...
    def import_csv(self, filepath, input_category, utc_offset: float, unit = "m", how: str="add", header=None, check_dublicates=False):        
        ...
        self.check_affiliation(input_category,self.VALID_CATEGORY)
        
        #check for non valid pressure units (GW,BP)
        if any(cat in input_category for cat in ("GW","BP")):
            self.check_affiliation([u.lower() for u in np.array(unit).flatten()],self.const['_pucf'].keys())
        ...
        header = self.zip_formatter(locations,input_category,unit)
        ...
        self.utc_offset.update(dict(self.zip_formatter(locations, utc_offset)))
